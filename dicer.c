...
static void sync_all_leds(struct dicer_t *d)
{
    size_t w;
    char *buf;
    size_t len;
...
static void event_decoded(struct deck_t *d, led_t led[NBUTTONS],
...
static void clear(struct controller_t *c)
{
    struct dicer_t *d = c->local;
...
static struct controller_type_t type = {
    .add_deck = add_deck,
    .realtime = realtime,
    .clear = clear,
};

int dicer_init(struct controller_t *c, struct rt_t *rt, const char *hw)
{
    size_t n;
    struct dicer_t *d;
